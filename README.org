...
***** Using a Non-Lisp?
      ...
      If you want to do SICP in another language it is possible possible (if
      slightly unhinged). You will greatly suffer if your choice doesn't support
      lexical closures, first-class functions and other features typical of
      newer lisps. You may even get a message *entirely* different from the one
