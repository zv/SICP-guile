...
straitforward Lisp functions with closures. Classes will permit you to solve
exercises quicker, write less code and be more satisfied with your final result.

The following are example base-classes for the primary classes, which allow
method introduced later later in the chapter such as ~process-new-value~ and
~process-forget-value~ to share implementation details regardless of if they are
operating on an ~adder~ or ~multiplier~.
